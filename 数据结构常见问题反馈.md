...
# [数据结构常见问题反馈(截至0411)](https://github.com/OneForward/support_cpp/blob/master/数据结构常见问题反馈.md)

### 变量或者对象的生存期与内存管理

- 定义数组时，数组大小使用变量。
- 数组没有初始化却被访问。（可能引发**SIGSEGV**内存异常的中断，或者无问题）
...
```c++
int* p; // p0 points to random position of memory
*p = 1; // trouble
...
- 函数存在无返回值的可能性。
  ...
  忘记写返回或者没有管辖到条件分支的所有情况。

  经常发生在**赋值运算符重载忘记返回*this**。
